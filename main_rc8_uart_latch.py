...
#   latch-091826.1    code 7 on a completed retract (a successful retract set
#                     no code, so the HUD stayed on 4); refetch ends at 0
#   latch-092226.1    BME280 enclosure monitoring for leak detection:
#                     EHUM / EDEW / EPRES / ETEMP on the HUD
#   latch-092426.1    neutral() guarded: a dead pigpio socket raised from
...
# ===========================================================================
# 082426: LATCH-RELEASE VARIANT of main_rc8_uart_parm.py.
#
# Identical to that file except for release_win(), which ends the release on
# the Hall sensor instead of a fixed timer, restoring the behaviour from the
# 01f64c0 build (2026-05-05). The servo drives only until the sensor confirms
# the line is out - typically 0.1-0.25 s - then goes neutral and the payload
# free-falls. RELEASE_SEC is demoted to a backstop: if the Hall never confirms,
# this behaves exactly like the direct-drive version.
#
# Everything else is unchanged: source ids, DATA96 chunking, GCS status, the
# pre-release recovery interlock, and the retract path.
# ===========================================================================
...
def release_win(servo, adc, cfg, st, stop_evt):
    ...
    if _ina["dev"] is not None:
        try:
            ...
            pass
    haucs_code(1, "release started, sampling", cfg)
    ...
    # exceeds RETRACT_TH, so a release that never paid out that far left the
                    ...
                    if ble.sdata.get("connection"):
                        try:
                            ...
                            if ok and len(do_list) > 0:
                                ...
                                haucs_code(11, "CAST COMPLETE but 0 samples", wParms,
