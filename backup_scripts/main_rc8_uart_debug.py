...
# CHANGED: UART-only setup. The Pi talks directly to the Cube on /dev/serial0.
# No UDP forwarding to a PC/GCS is used in this version.
GCS_UDP_OUT = None
...
# CHANGED: FSM is now triggered by RC input, not SERVO_OUTPUT_RAW
fsm_st = {
    "cycle_active": False,
...
# CHANGED: Trigger detection now uses MAVLink RC_CHANNELS.
# RC input channel to monitor from MAVLink RC_CHANNELS.
# This is 1-based: 8 means RC8 / chan8_raw.
# Change only this constant if you later want RC7, RC9, etc.
TRIGGER_RC_CH = 8
PWM_RELEASE = 1800
PWM_RETRACT = 1200
DEBOUNCE_S = 0.1
...
def rc_channel_value(msg, ch):
    # CHANGED: Replaces servo_out_value().
...
def handle_rc_channels(
    # CHANGED: Replaces handle_servo_output_raw().
    msg,
    fsm,
    ch=TRIGGER_RC_CH,
...
def mav_thread(stop_evt, q_winch, q_ble, q_mav, wincfg, winst, blest):
    ...
    try:
        ...
        # CHANGED: No GCS UDP connection. This version uses only the UART link.
        # Keep m_gcs as None so payload_link below resolves to m_fc.
        m_gcs = None
        ...
        # CHANGED: With GCS_UDP_OUT disabled, payload data is sent back over UART.
        ...
        while not stop_evt.is_set():
            ...
            if t == "GLOBAL_POSITION_INT":
                ...
                # CHANGED: debug print RC values
                print("RC_CHANNELS:", msg)
                # CHANGED: Monitor RC8 from RC_CHANNELS instead of SERVO_OUTPUT_RAW.
